...
?>
  ...
  <main class="flex flex-col lg:mt-32 mt-20 w-11/12 mx-auto">
    ...
    <?php
      ...
      if(!isset($_SESSION['exploreUsername'])) {
            echo '<input placeholder="Invite User" class="w-9/12 mt-6 lg:mt-0 h-1/4 bg-transparent text-gris border border-gris p-1 px-2 rounded" id="user_search_bar" name="input" oninput=instantResearch("../sources/dynamicUserSearch.php",1,'.$_SESSION['user']['id'].',1,'.$_GET['id'].','.$_SESSION['user']['id'].') />';
